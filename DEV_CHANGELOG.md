...
## 2026-08-20
- 需求 / 目标：将当前来店趋势与来店简报中的同期展示统一改为上期展示。
- 改动内容：来店趋势改用上月日数据和累计数据，移除当前展示中的同期 / 同比卡片、明细行和图例；来店简报仅保留本期与上期环比；来源说明更新为本期、上期共 4 张底表。底层同期工作表、线索同比逻辑和历史归档保留不变。
- 涉及文件：`scripts/build_dashboard.py`、`tests/test_build_dashboard.py`、`docs/data/dashboard.json`、`docs/data/dashboard.summary.json`、`docs/data/monthly/2026-08/`、`DEV_CHANGELOG.md`。
- 验证结果：来店专项测试通过，正式 dashboard 重建完成；全量单测 `120/120` 通过，Python 编译、前端脚本检查和 `git diff --check` 通过。
- 关联提交（如有）：待补充
- 备注：当前正式报表日期为 `2026-08-19`。
...
- 涉及文件：`scripts/fetch_daily_data.py`、`tests/test_fetch_daily_data.py`、`README.md`、`SCRIPTS.md`、`DEV_CHANGELOG.md`
- 关键命令：`python -B -X utf8 -m unittest discover -s tests -v`、基于 `.runtime/daily_update/20260730_20260731-142452` 回放 NEV / ICE 上期导出文件匹配。
- 验证结果：全量单测 `112/112` 通过；真实目录回放中 NEV 命中 `NEV上期-0630.xlsx`，ICE 精确命中 `来店上期-0730.xlsx`；两套命名方式均可用且歧义候选会被拒绝。
- 回滚方法：移除 `allow_period_suffix` 映射标记、周期后缀候选逻辑及对应测试、文档和本条日志。
- 关联提交（如有）：待补充
- 备注：本次未代替用户再次触发真实手动兜底更新。
...
- 涉及文件：`data/source/NEV+ICE_xsai.xlsm`、`data/source/NEV+ICE_ldai.xlsx`、`docs/data/dashboard.json`、`docs/data/dashboard.summary.json`、`docs/data/monthly/2026-06/dashboard.json`、`docs/data/monthly/2026-06/dashboard.summary.json`、`docs/data/monthly/index.json`、`DEV_CHANGELOG.md`
- 关键命令：`python -B -X utf8 scripts\build_dashboard.py ...`、`python -B -X utf8 -m py_compile scripts\build_dashboard.py`
- 验证结果：6/4 当前 dashboard 与 6 月归档的十五代轩逸目标序列均匹配截图；截至 6/4 累计目标 `4,633`，当日目标 `1,159`，月目标合计 `36,472`。
- 回滚方法：回退本条涉及的源工作簿、dashboard JSON、月度归档 JSON、index 与本条记录。
- 关联提交（如有）：待补充
- 备注：第一次提交推送后，本机静默更新写入了 6/4 源数据并生成了未带目标的本地 dashboard；本条用于把最新源数据重新套用目标 override 后发布。
...
- 关键命令：待补充
- 验证结果：代码层面已确认编号条目会在同一段落内单独换行，而不再依赖多个段落节点制造空隙。
- 回滚方法：回退本次 `docs/assets/app.js` 渲染逻辑修改。
- 关联提交（如有）：待补充
- 备注：这次是“保留条目层次”，不是回到之前那种一段一空行的排版。
...
## 2026-07-14 推送新探陆线索功能
- 需求 / 目标：将新探陆月度对照、线索趋势及每日简报功能推送到 GitHub `main` 分支。
- 改动内容：推送设计与实现提交；未纳入工作区无关未跟踪文件。
- 涉及文件：`DEV_CHANGELOG.md`
- 关键命令：`git push origin main`
- 验证结果：远端 `main` 已从 `754e9a7` 更新到 `5de09ea`，包含设计提交 `4d24793` 与实现提交 `5de09ea`，并触发 Pages workflow。
- 回滚方法：基于相关提交创建反向提交，不改写远端历史。
...
## 2026-09-10 15:35 探陆数据停更
- 需求 / 目标：自当前最新业务日后停止更新 2026 款探陆，简报与批量截图排除探陆，9 月保留现有趋势，10 月起移除趋势板块，同时保留旧数据。
- 改动内容：构建层统一过滤 `2026-09-09` 之后的探陆实际数据，9 月探陆 section 固定使用该日作为报表指针，`2026-10` 起不再生成该 section；每日简报删除探陆独立卡片；截图队列始终跳过 `new-pathfinder`。
- 数据结果：live 与 `2026-09` 归档已重建，探陆累计 / 当日新增线索保持 `833 / 77`，7—8 月归档和两本源工作簿未修改。
...
## 2026-09-16 全车系有效线索累计实绩口径核查
- 核查结论：业务日 `2026-09-15` 的累计实绩 `349,061` 为 `全国按日NEV` 有效线索 `177,859` 与 `全国按日ICE` 有效线索 `171,202` 之和。
- 口径说明：NEV 原始有效线索为 `178,318`，按探陆停更规则剔除 `2026-09-10 ~ 2026-09-15` 的 2026 款探陆有效线索 `459` 后为 `177,859`；`177,859 + 171,202 = 349,061`。
- 验证结果：源工作簿逐行复算、Dashboard 按日累计序列及摘要卡片三者一致；未修改代码或业务数据。

## 2026-09-16 探陆冻结规则历史核查
- 核查结论：`2026款探陆` 冻结在 `2026-09-09` 的规则于 `2026-09-10 15:36:27 +08:00` 加入，提交为 `f0be0e8`（`Retire Pathfinder dashboard updates`）。
- 需求来源：根据当日“网站今天之后不再更新探陆数据、简报去掉探陆、截图不截探陆、趋势图不更新、次月起移除探陆板块并保留旧数据”的要求，将当时最新完整业务日 `2026-09-09` 设为冻结日。
- 验证结果：`git blame`、提交详情及 `2026-09-10 15:35` 变更日志三处一致；未修改业务逻辑或数据。

## 2026-09-16 探陆独立板块与全车系汇总口径拆分
- 需求 / 目标：探陆仅停止更新并隐藏独立板块，全车系有效线索累计继续包含源表中的全部探陆数据。
- 根因：原实现进入构建器后先全局过滤 `2026-09-09` 之后的探陆行，导致独立板块冻结的同时，全车系有效线索也错误少计后续探陆。
- 改动内容：取消 NEV 源数据的全局冻结，只在构建 `new-pathfinder` 独立区块时截断到 `2026-09-09`；`2026-10` 起隐藏独立区块、简报不展示探陆独立卡片及截图跳过探陆的规则保持不变；补充回归测试锁定“独立区块冻结、全车系继续计入”两个口径。
- 数据结果：业务日 `2026-09-15` 的全车系累计有效线索由 `349,061` 恢复为 `349,520`，累计达成率 `39.0%`、同比 `-14.8%`、环比 `0.2%`、当日实绩 `23,586`；探陆独立板块修改前后均为累计新增线索 `836`、当日 `76`，报告日保持 `2026-09-09`。
- 验证结果：构建器专项测试 `49/49`、AI_Digest 全量测试 `132/132` 通过；Python 编译、Node 语法、JSON 解析与 `git diff --check` 均通过；live 与 `2026-09` 归档口径一致。

## 2026-09-16 全车系纳入空白车型数据
- 需求 / 目标：全车系有效线索汇总需计入 `全国按日NEV` 中车型列为空白的数据。
- 根因：`load_nev_daily()` 将空白车型行直接判定为无效行并跳过，导致这些独立线索未进入全车系聚合。
- 改动内容：空白车型行改为以空模型键保留；全车系聚合继续汇总所有模型键，NEV 独立趋势仍只精确匹配已配置车型，因此不会新增空白车型板块；同期表采用相同读取口径。
- 数据结果：业务日 `2026-09-15` 的本月空白车型有效线索为 `814`，全车系累计实绩由 `349,520` 更新为 `350,334`，累计达成率 `39.1%`、同比 `-14.6%`、环比 `0.2%`、当日实绩 `23,623`；探陆独立板块仍为累计 / 当日新增线索 `836 / 76`。
- 验证结果：专项测试 `4/4`、AI_Digest 全量测试 `134/134` 通过；Python 编译、Node 语法、JSON 解析与 `git diff --check` 均通过；live 与 `2026-09` 归档一致。
- 涉及文件：`scripts/build_dashboard.py`、`tests/test_build_dashboard.py`、`README.md`、`SCRIPTS.md`、live / `2026-09` Dashboard 数据及本日志。
- 关联提交（如有）：待补充

## 2026-09-16 历史归档空白车型口径回补
- 需求 / 目标：让已有历史月份的全车系有效线索与当前页面统一，纳入车型列为空白的 NEV 数据，同时保留当时的版式、同比 / 环比规则和元数据。
- 实现方法：为每个月定位最终归档提交，在隔离 Git worktree 中使用该提交的历史生成器与两本源 Excel；运行时仅补入空白车型读取逻辑生成候选结果，递归审计差异后只回填正式归档的 `lead-control` 与每日简报 `briefing`。
- 影响范围：5—6 月旧版全车系来自预汇总表，候选与正式归档业务数据零差异，无需修改；7—8 月从车型明细聚合，属于实际受影响月份。4 月月末快照不存在空白车型行。
- 数据结果：7 月累计实绩由 `711,908` 更新为 `713,233`，当日由 `29,785` 更新为 `29,833`，达成率 / 同比 / 环比更新为 `106.7% / -18.6% / -6.9%`；8 月累计实绩由 `745,073` 更新为 `746,565`，当日由 `26,975` 更新为 `27,016`，上期累计由 `691,276` 更新为 `692,420`，达成率 / 同比 / 环比更新为 `112.8% / -20.0% / 7.8%`。
