...
- 备注：
  ...
  - 来店环比按本月日号与上月日期对齐；本月日期超出上月天数时，当日环比回退使用上月最后一天，累计环比仍按实际上月日期去重，避免重复累计月末数据。本月全月结束时即为本月全月对比上月全月
  - `NEV 线索趋势` 会从 `全国按日NEV` 精确匹配车型 `2026款探陆`；探陆独立区块仅保留截至 `2026-09-09` 的历史数据，2026 年 9 月趋势固定停在该日，`2026-10` 起不再生成该区块。全车系有效线索继续按源表全量汇总，包含后续探陆数据及车型列为空白的 NEV 数据；空白车型不生成独立趋势板块。受影响的 `2026-07`、`2026-08` 历史归档已用各自月末 Git 源表快照回补，5—6 月旧版预汇总口径原本已包含该数据；原有 7—9 月历史归档不删除，批量截图始终跳过探陆
  - 每日简报固定展示全车系有效线索、NEV新增线索、来店简报三块；全车系月目标按报表月份读取 `config/dashboard_targets.json`（2026 年 7 月为 `668,262`），可由附魔工作台“手动兜底更新”工具维护；同比读取 `全国按日NEV-同期` 与 `全国按日ICE-同期`；累计环比按本月截至报告日与上月同日的累计值比较，本月日期超出上月天数时回退到上月最后一天，确保每日简报与趋势卡口径一致；全车系目标说明紧跟指标文字，NEV 目标说明显示在板块下方，两者月份均按报表月份自动生成
