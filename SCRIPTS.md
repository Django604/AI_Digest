...
- 备注：
  ...
  - `NEV 线索趋势` 会从 `全国按日NEV` 精确匹配车型 `2026款探陆`；探陆独立区块仅保留截至 `2026-09-09` 的历史数据，2026 年 9 月趋势固定停在该日，`2026-10` 起不再生成该区块。全车系有效线索继续按源表全量汇总，包含后续探陆数据及车型列为空白的 NEV 数据；空白车型不生成独立趋势板块。原有 7—9 月历史归档不删除，批量截图始终跳过探陆
