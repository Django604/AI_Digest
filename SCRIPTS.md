...
- 备注：
  ...
  - 每日简报固定展示全车系有效线索、NEV新增线索、2026款探陆线索、来店简报四块；全车系月目标按报表月份读取 `config/dashboard_targets.json`（2026 年 7 月为 `668,262`），可由附魔工作台“手动兜底更新”工具维护；同比读取 `全国按日NEV-同期` 与 `全国按日ICE-同期`，环比读取当前全国按日表中的上月数据
