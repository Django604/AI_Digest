...
- 备注：
  ...
  - 每日简报固定展示全车系有效线索、NEV新增线索、2026款探陆线索、来店简报四块；全车系月目标按报表月份读取 `config/dashboard_targets.json`（2026 年 7 月为 `668,262`），可由附魔工作台“手动兜底更新”工具维护；同比读取 `全国按日NEV-同期` 与 `全国按日ICE-同期`；累计环比按本月截至报告日与上月同日的累计值比较，本月日期超出上月天数时回退到上月最后一天，确保每日简报与趋势卡口径一致；全车系与 NEV 目标说明中的月份按报表月份自动生成
