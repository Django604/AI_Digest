...
`https://cdn.jsdelivr.net/gh/Django604/AI_Digest@main/docs/index.html`
...
jsDelivr 直接从公开 GitHub 仓库的 `main` 分支分发 `docs/` 静态文件。`index.html` 中现有相对路径继续加载同目录下的 `assets/`、`data/` 与 `data/monthly/`，无需修改前端路由或数据读取代码。
...
4. 推送后验证 jsDelivr 首页、JS、CSS、当前 dashboard JSON 和月度索引均返回 `HTTP 200`。
5. 使用真实浏览器打开 jsDelivr 主入口，确认页面渲染、当前数据、新探陆 section 和月份切换正常。
